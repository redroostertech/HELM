...
export function builderSlotPrompt(ctx: PromptContext, spec: TeamMemberSpec): string {
  ...
  return [
    ...
    '',
    '## Dev launchers (required — coordinate with other builders)',
    '',
    `Before BUILD_COMPLETE, ensure the repo root has TWO executable scripts:`,
    `\`install.sh\` (one-time setup) and \`run.sh\` (start the dev stack).`,
    `These are the canonical entry points used by HELM AppMonitor and human devs.`,
    '',
    `### ${ctx.repoPath}/install.sh`,
    `  • Mode: chmod +x; #!/usr/bin/env bash; \`set -e\``,
    `  • Idempotent dependency setup — safe to re-run anytime.`,
    `  • Installs every language/runtime dep the project needs (npm install,`,
    `    pip install, brew, etc.). Does NOT start any services.`,
    `  • If your project has nothing to install (rare), the script should`,
    `    still exist and exit 0 with a "no install needed" message.`,
    '',
    `### ${ctx.repoPath}/run.sh`,
    `  • Mode: chmod +x; #!/usr/bin/env bash; \`set -e\``,
    `  • Starts ALL services the project needs (api, frontend, gateway, etc.)`,
    `    with dev flags enabled.`,
    `  • Does NOT install dependencies — assumes install.sh has already run.`,
    `    If a required dep is missing, fail with a clear message pointing the`,
    `    user at \`./install.sh\`.`,
    `  • Echo each URL the user can hit ("Local: http://localhost:PORT" lines)`,
    `    so AppMonitor's URL auto-detection picks them up.`,
    `  • Multi-process: install a SIGINT/SIGTERM trap that kills the whole`,
    `    group so AppMonitor can stop everything with one signal.`,
    '',
    'Coordinate with other builders — only ONE install.sh and ONE run.sh exist',
    'in the repo. Use MSG-TO-{otherBuilder} to negotiate edits. The frontend',
    'builder typically owns the scripts; backend/infra builders contribute',
    'their install + start commands via MSG-TO. If unsure, the prd-refiner',
    'can arbitrate (MSG-TO-prd-refiner).',
    '',
    ...
    '    wait',
    '',
    `### ${ctx.repoPath}/.cruise/targets.json`,
    `  • Authoritative list of URLs the project exposes. HELM AppMonitor`,
    `    reads this and uses it to populate target chips above the embedded`,
    `    webview (instead of grabbing whichever URL prints first in stdout).`,
    `  • Format:`,
    '',
    '        {',
    '          "targets": [',
    '            { "name": "web",  "url": "http://localhost:3000", "primary": true,  "description": "Next.js UI" },',
    '            { "name": "api",  "url": "http://localhost:8000",                    "description": "FastAPI service" },',
    '            { "name": "docs", "url": "http://localhost:8000/docs",               "description": "OpenAPI Swagger UI" }',
    '          ]',
    '        }',
    '',
    `  • Exactly ONE target should have \`"primary": true\` — it's the URL`,
    `    the webview opens first. If you omit primary, the first entry wins.`,
    `  • Include every URL a human reviewer would want to visit: the UI, the`,
    `    api root, the api docs/swagger, any admin pages, any service health`,
    `    endpoints. AppMonitor renders each one as a clickable chip.`,
    '',
    '## Done marker',
    `When YOUR SLICE is complete, every assigned goal\'s acceptance criteria`,
    `are satisfied, install.sh + run.sh launch the project cleanly, AND you`,
    `have zero open tasks assigned to ${role}, emit:`,
...
export function reviewerSlotPrompt(ctx: PromptContext, spec: TeamMemberSpec): string {
  ...
  return [
    ...
    '',
    '## Verify the dev launchers (install.sh + run.sh)',
    '',
    `The repo MUST have BOTH of these executable scripts at the root:`,
    `  • ${ctx.repoPath}/install.sh — idempotent dependency setup, no service starts`,
    `  • ${ctx.repoPath}/run.sh — starts all services in dev mode, assumes install ran`,
    '',
    `Check each one:`,
    `  • Exists and is chmod +x.`,
    `  • Starts with #!/usr/bin/env bash and uses \`set -e\`.`,
    `  • install.sh actually installs every dep referenced by the build`,
    `    (npm install for any package.json, pip install for any requirements).`,
    `  • run.sh does NOT re-install — it should fail fast with a clear message`,
    `    if a dep is missing, pointing at ./install.sh.`,
    `  • run.sh echoes each "Local: http://..." URL so AppMonitor picks them up.`,
    `  • run.sh cleans up child processes on SIGINT/SIGTERM (look for a \`trap\`).`,
    '',
    `Also verify \`${ctx.repoPath}/.cruise/targets.json\` exists with:`,
    `  • At least one target.`,
    `  • Every URL the project actually serves (cross-reference run.sh output`,
    `    and the team plan — every "Local:" line should appear here).`,
    `  • Exactly one entry with "primary": true.`,
    '',
    'If any of those are missing or wrong, emit TASK-FOR markers to the',
    'appropriate builder(s) — frontend builder typically owns the scripts',
